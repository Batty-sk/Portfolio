    ...
    <section className='md:h-screen h-fit p-1 w-full text-black flex flex-col justify-center items-center rounded-bl-3xl rounded-br-3xl border-b-4  bg-[#fffad7] overflow-hidden' id='about'>
            ...
            <div className='content-container flex md:flex-row flex-col justify-center items-center gap-5 '>
                ...
                {SOCIAL_LINKS.map((x,i)=>(
                    <li key={i} className='social opacity-0 rounded-full p-1  bg-zinc-50 border shadow-md relative cursor-pointer z-0'><a href={x.link} target='_blank'> <Image src={x.img} height={30} width={30} alt='social'></Image></a></li>
                ))}
            </ul>
           </div>
